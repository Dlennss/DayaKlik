...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!pulsaCategory) {
    return buildPageMetadata({
      title: "Pulsa Operator | DayaKlik",
      description: "Isi pulsa operator favorit Anda di DayaKlik.",
      path: "/pulsa",
    });
  }
  ...
  return buildPageMetadata({
    title: `Isi Pulsa ${brandTitle} Online | DayaKlik`,
    description: lowestPrice
      ? `Isi pulsa ${brandTitle} online di DayaKlik dengan nominal lengkap. Harga mulai Rp ${lowestPrice.toLocaleString("id-ID")} dan transaksi cepat.`
      : `Isi pulsa ${brandTitle} online di DayaKlik dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
...
export default async function GuestPulsaBrandPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `Isi Pulsa ${brandTitle} Online | DayaKlik`,
    description: `Isi pulsa ${brandTitle} online di DayaKlik dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
  ...
  const faqJsonLd = buildFaqJsonLd([
    {
      question: `Apakah nominal pulsa ${brandTitle} di DayaKlik lengkap?`,
      answer: `Ya. DayaKlik menampilkan pilihan nominal pulsa ${brandTitle} yang aktif agar pembeli bisa memilih sesuai kebutuhan.`,
    },
    {
      question: `Bagaimana cara beli pulsa ${brandTitle} di DayaKlik?`,
      answer: `Pilih nominal pulsa ${brandTitle}, masukkan nomor tujuan, lalu lanjutkan pembayaran sesuai metode yang tersedia.`,
    },
    {
      question: `Apakah pembelian pulsa ${brandTitle} di DayaKlik bisa untuk calon member dan member?`,
      answer: `Bisa. Halaman ini bisa dipakai pembeli umum, dan member DayaKlik juga bisa memanfaatkan pilihan produk yang sama untuk transaksi harian.`,
    },
  ]);
